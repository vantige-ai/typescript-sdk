...
jest.mock('../client/vantige-client', () => {
  const mockClientInstance = {
    getKeyInfo: jest.fn().mockReturnValue({ environment: 'test', maskedKey: 'vk_test_****' }),
  };
  
  // Create a proper mock constructor that returns instances
  const MockVantigeClient = jest.fn().mockImplementation(() => mockClientInstance) as any;
  
  // Make the mock constructor itself have the static methods
  MockVantigeClient.createTestClient = jest.fn().mockImplementation(() => mockClientInstance);
  MockVantigeClient.createLiveClient = jest.fn().mockImplementation(() => mockClientInstance);
  MockVantigeClient.validateApiKey = jest.fn().mockImplementation((apiKey: string) => {
